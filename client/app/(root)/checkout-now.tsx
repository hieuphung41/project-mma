...
import { useCreateOrderNowMutation, useGetOrdersByUserIdQuery } from "@/redux/api/orderApiSlice";
    ...
    try {
      if (paymentMethod === "VNPAY") {
        ...
        if (response?.data?.message === "Success") {
          ...
          const orderResponse = await createOrderNow(orderDataWithCode);
  ...
  const handlePayment = async () => {
    try {
      const response = await fetch(
        "http://192.168.0.101:5500/api/v1/checkout/create_payment_url",
        {
          method: "POST",
